    ...
    public class UserController : ApiController
    {
        ...
        public string GetMonitorUserInfo()
        {
            ...
            if (!string.IsNullOrEmpty(datas))
            {
                ...
                for (int i = 0; i < rows.Length; i++)
                {
                    ...
                    if (user != null)
                    {
                        string maps = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM map_user_role WHERE userid={0} AND ztm={1}", user.Id, (int)MODEL.Enum.State.InUse));
        ...
        public string DeleteUser()
        {
            ...
            if (count > 0)
            {
                ...
                int mapcount = PostgresqlHelper.QueryResultCount(pgsqlConnection, string.Format("SELECT *FROM map_user_role WHERE userid={0} AND ztm={1}", id, (int)MODEL.Enum.State.InUse));
                if (mapcount > 0)
                {
                    mapcount = PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE map_user_role SET ztm={0} WHERE userid={1}", (int)MODEL.Enum.State.NoUse, id));
