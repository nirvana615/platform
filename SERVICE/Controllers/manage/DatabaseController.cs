...
namespace SERVICE.Controllers
{
    ...
    public class DatabaseController : ApiController
    {
        ...
        {
            string data = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM manage_database WHERE ztm={0} ORDER BY id ASC", (int)MODEL.Enum.State.InUse));
        ...
        public string AddDatabase()
        {
            ...
            if (dblx == ((int)MODEL.Enum.DbType.SQLite).ToString())
            {
                if (!string.IsNullOrEmpty(dbadd))
                {
                    ...
                    int id = PostgresqlHelper.InsertDataReturnID(pgsqlConnection, "INSERT INTO manage_database (dblx,dbadd,cjsj,ztm,bz) VALUES" + value);
                    if (id != -1)
                    {
                        return "创建成功！";
                    }
            ...
            else
            {
                if (!string.IsNullOrEmpty(dbip) && !string.IsNullOrEmpty(dbport) && !string.IsNullOrEmpty(dbname) && !string.IsNullOrEmpty(dbuser) && !string.IsNullOrEmpty(dbpw))
                {
                    ...
                    int id = PostgresqlHelper.InsertDataReturnID(pgsqlConnection, "INSERT INTO manage_database (dblx,dbip,dbport,dbname,dbuser,dbpw,dbconn,cjsj,ztm,bz) VALUES" + value);
                    if (id != -1)
                    {
                        return "创建成功！";
                    }
        ...
        public string UpdateDatabase()
        {
            ...
            int count = PostgresqlHelper.QueryResultCount(pgsqlConnection, string.Format("SELECT *FROM manage_database WHERE id={0} AND ztm={1}", id, (int)MODEL.Enum.State.InUse));
            if (count > 0)
            {
                if (dblx == ((int)MODEL.Enum.DbType.SQLite).ToString())
                {
                    count = 0;
                    count = PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE manage_database SET dblx={0},dbadd={1},bz={2} WHERE id={3} AND ztm={4}", dblx, SQLHelper.UpdateString(dbadd), SQLHelper.UpdateString(bz), id, (int)MODEL.Enum.State.InUse));
        ...
        public string DeleteDatabase()
        {
            ...
            if (count > 0)
            {
                count = PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE manage_database SET ztm={0} WHERE id={1}", (int)MODEL.Enum.State.NoUse, id));
                int mapcount = PostgresqlHelper.QueryResultCount(pgsqlConnection, string.Format("SELECT *FROM map_device_database WHERE dbid={0} AND ztm={1}", id, (int)MODEL.Enum.State.InUse));
                if (mapcount > 0)
                {
                    mapcount = PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE map_device_database SET ztm={0} WHERE dbid={1}", (int)MODEL.Enum.State.NoUse, id));
