    ...
    public class DataController : ApiController
    {
        ...
        private string GetAutoDatabyDateTime(int id, string type, string time, string userbsms)
        {
            if (type == MODEL.Enum.AutoDeviceType.GNSS.GetRemark())
            {
                ...
                {
                    //获取设备
                    Device device = ParseManageHelper.ParseDevice(PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM manage_device WHERE id={0} AND sblx={1} AND bsm{2} AND ztm={3}", mapMonitorDevice.DeviceId, (int)MODEL.Enum.AutoDeviceType.GNSS, userbsms, (int)MODEL.Enum.State.InUse)));
                    if (device != null)
                    {
                        MapDeviceValue mapDeviceValue = ParseMonitorHelper.ParseMapDeviceValue(PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM map_device_value WHERE deviceid={0} AND ztm={1}", device.Id, (int)MODEL.Enum.State.InUse)));

                        if (mapDeviceValue != null)
                        {
                            //获取初始值
                            Value value = ParseMonitorHelper.ParseValue(PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM business_value WHERE id={0} AND ztm={1} AND bsm{2}", mapDeviceValue.ValueId, (int)MODEL.Enum.State.InUse, userbsms)));
                            if (value != null)
                            {
                                ...
                                if (gnssValue != null)
                                {
                                    ...
                                    string data = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT temp.dx,temp.dy,SQRT(temp.dx*temp.dx+temp.dy*temp.dy) dxy,temp.dh,temp.datetime,temp.datatype FROM (SELECT (x-CAST((SELECT value->>'X' FROM business_value WHERE id={0} AND ztm={1}) as numeric))*1000 dx,(y-CAST((SELECT value->>'Y' FROM business_value WHERE id={0} AND ztm={1}) as numeric))*1000 dy,(h-CAST((SELECT value->>'H' FROM business_value WHERE id={0} AND ztm={1}) as numeric))*1000 dh,gcsj datetime,lb datatype FROM business_data_gnss WHERE code='{2}' AND {3} AND bsm{4} ORDER BY gcsj) temp", value.Id, (int)MODEL.Enum.State.InUse, device.Code, time, userbsms));
                        ...
                        {
                            //无初始值，则将第一条数据作为初始值
                            GNSS gnss = ParseMonitorHelper.ParseGNSS(PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM business_data_gnss WHERE code='{0}' AND bsm{1} ORDER BY gcsj ASC LIMIT 1", device.Code, userbsms)));
                            if (gnss != null)
                            {
                                // dx dy dxy dh time type
                                string data = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT temp.dx,temp.dy,SQRT(temp.dx*temp.dx+temp.dy*temp.dy) dxy,temp.dh,temp.datetime,temp.datatype FROM (SELECT (x-{0})*1000 dx,(y-{1})*1000 dy,(h-{2})*1000 dh,gcsj datetime,lb datatype FROM business_data_gnss WHERE code='{3}' AND {4} AND bsm{5} ORDER BY gcsj) temp", gnss.X, gnss.Y, gnss.H, device.Code, time, userbsms));
            ...
            else if (type == MODEL.Enum.AutoDeviceType.LF.GetRemark())
            {
                ...
                if (mapMonitorDevice != null)
                {
                    Device device = ParseManageHelper.ParseDevice(PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM manage_device WHERE id={0} AND sblx={1} AND bsm{2} AND ztm={3}", mapMonitorDevice.DeviceId, (int)MODEL.Enum.AutoDeviceType.LF, userbsms, (int)MODEL.Enum.State.InUse)));
                    if (device != null)
                    {
                        //TODO获取初始值

                        string data = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT value,gcsj,lb FROM business_data_lf WHERE code='{0}' AND {1} AND bsm{2} ORDER BY gcsj", device.Code, time, userbsms));
                        if (!string.IsNullOrEmpty(data))
                        {
                            string[] rows = data.Split(new char[] {COM.ConstHelper.rowSplit});
                            if (rows.Length > 0)
            ...
            else if (type == MODEL.Enum.AutoDeviceType.QJ.GetRemark())
            {
                ...
                if (mapMonitorDevice != null)
                {
                    Device device = ParseManageHelper.ParseDevice(PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM manage_device WHERE id={0} AND sblx={1} AND bsm{2} AND ztm={3}", mapMonitorDevice.DeviceId, (int)MODEL.Enum.AutoDeviceType.QJ, userbsms, (int)MODEL.Enum.State.InUse)));
                    if (device != null)
                    {
                        //TODO获取初始值

                        string data = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT x,y,z,gcsj,lb FROM business_data_qj WHERE code='{0}' AND {1} AND bsm{2} ORDER BY gcsj", device.Code, time, userbsms));
                        if (!string.IsNullOrEmpty(data))
                        {
                            string[] rows = data.Split(new char[] {COM.ConstHelper.rowSplit});
                            if (rows.Length > 0)
            ...
            else if (type == MODEL.Enum.AutoDeviceType.SBWY.GetRemark())
            {
                ...
                {
                    //获取设备
                    Device device = ParseManageHelper.ParseDevice(PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM manage_device WHERE id={0} AND sblx={1} AND bsm{2} AND ztm={3}", mapMonitorDevice.DeviceId, (int)MODEL.Enum.AutoDeviceType.SBWY, userbsms, (int)MODEL.Enum.State.InUse)));
                    if (device != null)
                    {
                        string data = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT x,y,z,gcsj,lb FROM business_data_sbwy WHERE code='{0}' AND bsm{1} AND {2} ORDER BY gcsj", device.Code, userbsms, time));
                        if (!string.IsNullOrEmpty(data))
                        {
                            string[] rows = data.Split(new char[] {COM.ConstHelper.rowSplit});
                            if (rows.Length > 0)
            ...
            else if (type == MODEL.Enum.AutoDeviceType.WATER.GetRemark())
            {
                ...
                //获取监测点
                Monitor monitor = ParseMonitorHelper.ParseMonitor(PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM business_monitor WHERE id={0} AND bsm{1} AND ztm={2}", id, userbsms, (int)MODEL.Enum.State.InUse)));
                if (monitor != null)
                {
                    //根据监测点号获取设备
                    MapMonitorDevice mapMonitorDevice = ParseMonitorHelper.ParseMapMonitorDevice(PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM map_monitor_device WHERE monitorid={0} AND ztm={1}", id, (int)MODEL.Enum.State.InUse)));
                    ...
                    {
                        //获取设备
                        Device device = ParseManageHelper.ParseDevice(PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM manage_device WHERE id={0} AND sblx={1} AND bsm{2} AND ztm={3}", mapMonitorDevice.DeviceId, (int)MODEL.Enum.AutoDeviceType.WATER, userbsms, (int)MODEL.Enum.State.InUse)));
                        if (device != null)
                        {
                            string data = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT value,gcsj,lb FROM business_data_water WHERE code='{0}' AND bsm{1} AND {2} ORDER BY gcsj", device.Code, userbsms, time));
                            if (!string.IsNullOrEmpty(data))
                            {
                                string[] rows = data.Split(new char[] {COM.ConstHelper.rowSplit});
                                if (rows.Length > 0)
            ...
            else if (type == MODEL.Enum.AutoDeviceType.YL.GetRemark())
            {
                ...
                if (mapMonitorDevice != null)
                {
                    Device device = ParseManageHelper.ParseDevice(PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM manage_device WHERE id={0} AND sblx={1} AND bsm{2} AND ztm={3}", mapMonitorDevice.DeviceId, (int)MODEL.Enum.AutoDeviceType.YL, userbsms, (int)MODEL.Enum.State.InUse)));
                    if (device != null)
                    {
                        //TODO获取初始值

                        string data = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT value,gcsj,lb FROM business_data_yl WHERE code='{0}' AND bsm{1} AND {2} ORDER BY gcsj", device.Code, userbsms, time));
                        if (!string.IsNullOrEmpty(data))
                        {
                            string[] rows = data.Split(new char[] {COM.ConstHelper.rowSplit});
                            if (rows.Length > 0)
            ...
            else if (type == MODEL.Enum.AutoDeviceType.RAIN.GetRemark())
            {
                ...
                {
                    //获取设备
                    Device device = ParseManageHelper.ParseDevice(PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM manage_device WHERE id={0} AND sblx={1} AND bsm{2} AND ztm={3}", mapMonitorDevice.DeviceId, (int)MODEL.Enum.AutoDeviceType.RAIN, userbsms, (int)MODEL.Enum.State.InUse)));
                    if (device != null)
                    {
                        string data = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT sum(value),substring(gcsj,0,11) FROM business_data_rain WHERE code='{0}' AND bsm{1} AND {2} GROUP BY substring(gcsj,0,11) ORDER BY substring(gcsj,0,11)", device.Code, userbsms, time));
                        if (!string.IsNullOrEmpty(data))
                        {
                            string[] rows = data.Split(new char[] {COM.ConstHelper.rowSplit});
                            if (rows.Length > 0)
