...
namespace SERVICE.Controllers
{
    ...
    public class ProjectController : ApiController
    {
        ...
        public string GetProjectList()
        {
            ...
            string datas = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM business_project WHERE ztm={0} ORDER BY id ASC", (int)MODEL.Enum.State.InUse));
            ...
            {
                Project project = ParseMonitorHelper.ParseProject(rows[i]);
                if (project != null)
                {
        ...
        public string GetUserProjectList(string cookie)
        {
            ...
            if (cookieResult == COM.CookieHelper.CookieResult.SuccessCookkie)
            {
                //有效cookie
                List<Project> projectList = new List<Project>();
                string data = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM business_project WHERE bsm{0} AND ztm={1} ORDER BY id ASC", userbsms, (int)MODEL.Enum.State.InUse));
        ...
        public string GetProjectInfo(int id, string cookie)
        {
            ...
            if (cookieResult == COM.CookieHelper.CookieResult.SuccessCookkie)
            {
                ProjectString projectString = ParseMonitorHelper.ParseProjectString(PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM business_project WHERE id={0} AND ztm={1} AND bsm{2}", id, (int)MODEL.Enum.State.InUse, userbsms)));
            ...
            if (cookieResult == COM.CookieHelper.CookieResult.SuccessCookkie)
            {
                    ...
                    && !string.IsNullOrEmpty(zhdtybh))
                {
                    ...
                    if (!string.IsNullOrEmpty(xmbm))
                    {
                        ...
                        int id = PostgresqlHelper.InsertDataReturnID(pgsqlConnection, "INSERT INTO business_project (xmmc,xmbm,zxjd,zxwd,srid,xzqbm,xmwz,xmkssj,xmjssj,zhdmc,zhdtybh,zhlx,qtwx,cjsj,bsm,ztm,bz) VALUES" + value);
                        if (id != -1)
                        {
                            if (!string.IsNullOrEmpty(xmlx))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET xmlx={0} WHERE id={1}", xmlx, id));
                            }

                            if (!string.IsNullOrEmpty(xmlb))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET xmlb={0} WHERE id={1}", xmlb, id));
                            }

                            if (!string.IsNullOrEmpty(zhdj))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET zhdj={0} WHERE id={1}", zhdj, id));
                            }

                            if (!string.IsNullOrEmpty(zhxq))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET zhxq={0} WHERE id={1}", zhxq, id));
                            }

                            if (!string.IsNullOrEmpty(jcjb))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET jcjb={0} WHERE id={1}", jcjb, id));
                            }

                            if (!string.IsNullOrEmpty(jcsd))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET jcsd={0} WHERE id={1}", jcsd, id));
                            }

                            if (!string.IsNullOrEmpty(yjjb))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET yjjb={0} WHERE id={1}", yjjb, id));
                            }

                            if (!string.IsNullOrEmpty(sfkq))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET sfkq={0} WHERE id={1}", sfkq, id));
                            }

                            if (!string.IsNullOrEmpty(sfss))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET sfss={0} WHERE id={1}", sfss, id));
                            }

                            if (!string.IsNullOrEmpty(sfjs))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET sfjs={0} WHERE id={1}", sfjs, id));
                            }

                            if (!string.IsNullOrEmpty(mj))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET mj={0} WHERE id={1}", mj, id));
                            }

                            if (!string.IsNullOrEmpty(mjdw))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET mjdw={0} WHERE id={1}", mjdw, id));
                            }

                            if (!string.IsNullOrEmpty(tj))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET tj={0} WHERE id={1}", tj, id));
                            }

                            if (!string.IsNullOrEmpty(tjdw))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET tjdw={0} WHERE id={1}", tjdw, id));
                            }

                            if (!string.IsNullOrEmpty(wxhs))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET wxhs={0} WHERE id={1}", wxhs, id));
                            }

                            if (!string.IsNullOrEmpty(wxrs))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET wxrs={0} WHERE id={1}", wxrs, id));
                            }

                            if (!string.IsNullOrEmpty(wxfwmj))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET wxfwmj={0} WHERE id={1}", wxfwmj, id));
                            }
        ...
        public string UpdateProject()
        {
            ...
            if (cookieResult == COM.CookieHelper.CookieResult.SuccessCookkie)
            {
                int count = PostgresqlHelper.QueryResultCount(pgsqlConnection, string.Format("SELECT *FROM business_project WHERE id={0} AND bsm{1} AND ztm={2}", id, userbsms, (int)MODEL.Enum.State.InUse));
                if (count == 1)
                {
                    if (!string.IsNullOrEmpty(xmmc)
                    && !string.IsNullOrEmpty(zxjd)
                    ...
                    && !string.IsNullOrEmpty(zhdmc))
                    {
                        int updatecount = PostgresqlHelper.UpdateData(pgsqlConnection, string.Format(
                             "UPDATE business_project SET xmmc={0},zxjd={1},zxwd={2},srid={3},xzqbm={4},xmwz={5},xmkssj={6},xmjssj={7},zhdmc={8},zhdtybh={9},zhlx={10},qtwx={11},bz={12} WHERE id={13} AND bsm{14} AND ztm={15}",
                             SQLHelper.UpdateString(xmmc),
                             zxjd,
                             zxwd,
                             srid,
                        ...
                        if (updatecount == 1)
                        {
                            if (!string.IsNullOrEmpty(xmlx))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET xmlx={0} WHERE id={1} AND bsm{2} AND ztm={3}", xmlx, id, userbsms, (int)MODEL.Enum.State.InUse));
                            }

                            if (!string.IsNullOrEmpty(xmlb))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET xmlb={0} WHERE id={1} AND bsm{2} AND ztm={3}", xmlb, id, userbsms, (int)MODEL.Enum.State.InUse));
                            }

                            if (!string.IsNullOrEmpty(zhdj))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET zhdj={0} WHERE id={1} AND bsm{2} AND ztm={3}", zhdj, id, userbsms, (int)MODEL.Enum.State.InUse));
                            }

                            if (!string.IsNullOrEmpty(zhxq))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET zhxq={0} WHERE id={1} AND bsm{2} AND ztm={3}", zhxq, id, userbsms, (int)MODEL.Enum.State.InUse));
                            }

                            if (!string.IsNullOrEmpty(jcjb))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET jcjb={0} WHERE id={1} AND bsm{2} AND ztm={3}", jcjb, id, userbsms, (int)MODEL.Enum.State.InUse));
                            }

                            if (!string.IsNullOrEmpty(jcsd))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET jcsd={0} WHERE id={1} AND bsm{2} AND ztm={3}", jcsd, id, userbsms, (int)MODEL.Enum.State.InUse));
                            }

                            if (!string.IsNullOrEmpty(yjjb))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET yjjb={0} WHERE id={1} AND bsm{2} AND ztm={3}", yjjb, id, userbsms, (int)MODEL.Enum.State.InUse));
                            }

                            if (!string.IsNullOrEmpty(sfkq))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET sfkq={0} WHERE id={1} AND bsm{2} AND ztm={3}", sfkq, id, userbsms, (int)MODEL.Enum.State.InUse));
                            }

                            if (!string.IsNullOrEmpty(sfss))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET sfss={0} WHERE id={1} AND bsm{2} AND ztm={3}", sfss, id, userbsms, (int)MODEL.Enum.State.InUse));
                            }

                            if (!string.IsNullOrEmpty(sfjs))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET sfjs={0} WHERE id={1} AND bsm{2} AND ztm={3}", sfjs, id, userbsms, (int)MODEL.Enum.State.InUse));
                            }

                            if (!string.IsNullOrEmpty(mj))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET mj={0} WHERE id={1} AND bsm{2} AND ztm={3}", mj, id, userbsms, (int)MODEL.Enum.State.InUse));
                            }

                            if (!string.IsNullOrEmpty(mjdw))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET mjdw={0} WHERE id={1} AND bsm{2} AND ztm={3}", mjdw, id, userbsms, (int)MODEL.Enum.State.InUse));
                            }

                            if (!string.IsNullOrEmpty(tj))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET tj={0} WHERE id={1} AND bsm{2} AND ztm={3}", tj, id, userbsms, (int)MODEL.Enum.State.InUse));
                            }

                            if (!string.IsNullOrEmpty(tjdw))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET tjdw={0} WHERE id={1} AND bsm{2} AND ztm={3}", tjdw, id, userbsms, (int)MODEL.Enum.State.InUse));
                            }

                            if (!string.IsNullOrEmpty(wxhs))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET wxhs={0} WHERE id={1} AND bsm{2} AND ztm={3}", wxhs, id, userbsms, (int)MODEL.Enum.State.InUse));
                            }

                            if (!string.IsNullOrEmpty(wxrs))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET wxrs={0} WHERE id={1} AND bsm{2} AND ztm={3}", wxrs, id, userbsms, (int)MODEL.Enum.State.InUse));
                            }

                            if (!string.IsNullOrEmpty(wxfwmj))
                            {
                                PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET wxfwmj={0} WHERE id={1} AND bsm{2} AND ztm={3}", wxfwmj, id, userbsms, (int)MODEL.Enum.State.InUse));
                            }
        ...
        public string DeleteProject()
        {
            ...
            if (cookieResult == COM.CookieHelper.CookieResult.SuccessCookkie)
            {
                int updatecount = PostgresqlHelper.UpdateData(pgsqlConnection, string.Format("UPDATE business_project SET ztm={0} WHERE id={1}", (int)MODEL.Enum.State.NoUse, id));
                if (updatecount == 1)
                {
        ...
        private string CreateProjectCode(string xjxzq, string type)
        {
            ...
            {
                string data = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM business_project WHERE xzqbm='{0}' AND zhlx={1}", xjxzq, type));
