        ...
        public string GetLayerInfo(int id, string cookie)
        {
            ...
            if (cookieResult == COM.CookieHelper.CookieResult.SuccessCookkie)
            {
                ...
                if (project != null)
                {
                    ...
                    string data = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM map_project_survey WHERE projectid={0} AND type={1} AND role={2} AND ztm={3}", id, (int)MODEL.Enum.SurveyDataType.Model, (int)MODEL.Enum.SystemRole.Monitor, (int)MODEL.Enum.State.InUse));
                    if (!string.IsNullOrEmpty(data))
                    {
                        ...
                        for (int i = 0; i < rows.Length; i++)
                        {
                            ...
                            {
                                SurModel surModel = ParseMonitorHelper.ParseSurModel(PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM survey_model WHERE id={0} AND bsm{1} AND ztm={2} AND mxjb={3}", mapProjectSurvey.SurveyId, userbsms, (int)MODEL.Enum.State.InUse, (int)MODEL.Enum.ModelLevel.Whole)));
                                if (surModel != null)
                                {
                    ...
                    #region
                    data = string.Empty;
                    data = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM map_project_disaster WHERE projectid={0} AND ztm={1}", id, (int)MODEL.Enum.State.InUse));
                    if (!string.IsNullOrEmpty(data))
                    {
                    ...
                    if (!string.IsNullOrEmpty(data))
                    {
                        ...
                        for (int i = 0; i < rows.Length; i++)
                        {
                            ...
                            if (mapProjectDisaster != null)
                            {
                                ...
                                {
                                    data = string.Empty;
                                    data = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM map_disaster_section WHERE disasterid={0} AND ztm={1}", disaster.Id, (int)MODEL.Enum.State.InUse));
                                    if (!string.IsNullOrEmpty(data))
                                    {
                                        ...
                                        for (int j = 0; j < rows1.Length; j++)
                                        {
                                            ...
                                            if (mapDisasterSection != null)
                                            {
                                                ...
                                                if (sectionString != null)
                                                {
                                                    ...
                                                    data = string.Empty;
                                                    data = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM map_section_monitor WHERE sectionid={0} AND ztm={1}", sectionString.Id, (int)MODEL.Enum.State.InUse));
                                                    if (!string.IsNullOrEmpty(data))
                                                    {
                                                        ...
                                                        for (int h = 0; h < rows2.Length; h++)
                                                        {
                                                            ...
                                                            {
                                                                MonitorString monitorString = ParseMonitorHelper.ParseMonitorString(PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM business_monitor WHERE id={0} AND bsm{1} AND ztm={2}", mapSectiontMonitor.MonitorId, userbsms, (int)MODEL.Enum.State.InUse)));
                                                                if (monitorString != null)
                                                                {
                                                                    ...
                                                                    PointXYZ XYZ = Execute.xyh2XYZ(Convert.ToDouble(monitorString.PMWZX), Convert.ToDouble(monitorString.PMWZY), Convert.ToDouble(monitorString.GC), monitorString.KJCK);
                                                                    if (XYZ != null)
                                                                    {
                                    ...
                                    data = string.Empty;
                                    data = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM map_disaster_monitor WHERE disasterid={0} AND ztm={1}", disaster.Id, (int)MODEL.Enum.State.InUse));
                                    if (!string.IsNullOrEmpty(data))
                                    {
                                        ...
                                        for (int k = 0; k < rows3.Length; k++)
                                        {
                                            ...
                                            {
                                                MonitorString monitorString = ParseMonitorHelper.ParseMonitorString(PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM business_monitor WHERE id={0} AND bsm{1} AND ztm={2}", mapDisasterMonitor.MonitorId, userbsms, (int)MODEL.Enum.State.InUse)));
                                                bool isIn = false;
                    ...
                    data = string.Empty;
                    data = PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM map_project_monitor WHERE projectid={0} AND ztm={1}", id, (int)MODEL.Enum.State.InUse));
                    if (!string.IsNullOrEmpty(data))
                    {
                        ...
                        for (int i = 0; i < rows4.Length; i++)
                        {
                            ...
                            {
                                MonitorString monitorString = ParseMonitorHelper.ParseMonitorString(PostgresqlHelper.QueryData(pgsqlConnection, string.Format("SELECT *FROM business_monitor WHERE id={0} AND bsm{1} AND ztm={2}", mapProjectMonitor.MonitorId, userbsms, (int)MODEL.Enum.State.InUse)));
                                if (monitorString != null)
                                {
                                    MonitorPointLayer monitorPointLayer = new MonitorPointLayer();
                                    monitorPointLayer.Id = monitorString.Id;
